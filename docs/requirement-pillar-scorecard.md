...
| **Multi-tenancy (data)** | Tenant isolation, hybrid DB, audit (`multi-tenant-architecture.md`, `multi-tenancy-security-strategy.md`) | ✅ Enforced at DB (2026-06-20) | RLS on 23 tables (ENABLE/FORCE + `tenant_isolation` policy), restricted `app_runtime` role, audited platform bypass; proven by `rls-isolation-check.sql`. Runtime cutover (app → `app_runtime`) pending — see ADR-004. Earlier "isolation not enforced" gap is closed at the DB layer. |
