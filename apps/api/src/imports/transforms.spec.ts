...
describe('applyTransform (F2)', () => {
  ...
  it('parses DD/MM/YYYY and YYYY-MM-DD to ISO, flags bad dates', () => {
    expect(
      applyTransform('date_parse', { format: 'DD/MM/YYYY' }, '05/03/2012')
        .value,
    ).toBe('2012-03-05');
    expect(applyTransform('date_parse', {}, '2011-10-10').value).toBe(
      '2011-10-10',
    );
  ...
  it('constant / split / lookup', () => {
    expect(applyTransform('constant', { value: 'NG' }, 'ignored').value).toBe(
      'NG',
    );
    expect(
      applyTransform('split', { separator: ' ', index: 1 }, 'Ada Okafor').value,
    ).toBe('Okafor');
    expect(
      applyTransform('lookup', { table: { M: 'male', F: 'female' } }, 'F')
        .value,
    ).toBe('female');
    expect(
      applyTransform('lookup', { table: {}, default: 'undisclosed' }, 'x')
        .value,
    ).toBe('undisclosed');
  });
});
