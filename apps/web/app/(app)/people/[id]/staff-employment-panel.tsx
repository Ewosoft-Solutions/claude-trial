...
export function StaffEmploymentPanel({
  ...
  /**
   * Employment rows resolved on the SERVER, with the rest of the tab.
   *
   * Without them this panel mounts empty and fetches, so the route's skeleton
   * was replaced by the panel's own — a second, smaller loader mid-wait. The
  ...
  const [rows, setRows] = React.useState<Employment[] | null>(
    initialRows ?? null,
  );
