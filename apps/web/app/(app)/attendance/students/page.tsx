...
import {
  ...
  type AttendanceRow,
} from './student-attendance-client';
...
export default async function StudentAttendancePage() {
  ...
  const [studentData, attendanceData] = await Promise.all([
    ...
    serverApiGet<ApiAttendanceRecord[]>(
      `/attendance?from=${from.toISOString().slice(0, 10)}`,
    ),
  ]);
  ...
  const rows: AttendanceRow[] = Array.from(grouped.entries()).map(
    ([studentId, totals]) => {
