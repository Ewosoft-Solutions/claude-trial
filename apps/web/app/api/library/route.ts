...
export async function GET(req: NextRequest) {
  if (!API_BASE) {
    return NextResponse.json({ books: [], mock: true });
  }
  try {
    const qs = req.nextUrl.searchParams.toString();
    const data = await apiClient.get(`/library/books${qs ? `?${qs}` : ''}`, authHeader(req));
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const data = await apiClient.post('/library/books', body, authHeader(req));
