...
**Foundations ready to build:** `F5` (delivery) · `F8` (Aurora patterns). _(`F7` (search) **claimed** by claude → `feat/F7-directory-search`. `F3` jobs/outbox + `F1` Person + `F4` documents + `F2` import all **done** — [PR #42](https://github.com/Ewosoft-Solutions/claude-trial/pull/42) merged to `main`. **WB1 (People directory)** now unblocked — needs F1 + F7 + F8.)_
...
| F7  | Governed **search + saved-views + URL-state directory** pattern                                                   | #105,#27    | M      | —          | claude | claimed          |
