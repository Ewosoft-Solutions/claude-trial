...
10. **Governed UI + domain routing.** A screen that lists an entity uses the governed **`DirectoryTable`** with the **Pattern-B toolbar** (search / filters / saved views), a **`StatGrid`** summary, and `ShellMain` + `PageHeader` — never a bespoke `<table>` + hand-rolled search. Quick-look is a **`Sheet` drawer**; deep work is a **full route**. **Surfaces** (§3): a **modal** blocks and never scrolls (confirmations, step-up, one decision ≤ 5 fields); every create/edit is a **drawer**; an action or form never gets an inline in-page panel that toggles open (an always-present editable row in the object's own table — the invoice lines entry row — is content, not a panel). State renders through the semantic **`StatusBadge`** tones, never ad-hoc colours. **Routing:** a screen lives under its domain segment and the nav label matches — the student lifecycle **including admissions** (`/students/admissions`) under `/students/*`, academics under `/academics/*`, finance under `/finance/*`. _Enforcement:_ DoD (§5) + review against [`docs/frontend-conventions.md`](docs/frontend-conventions.md); a lint gate for the table/route idioms is a tracked follow-up. _(Born from admissions shipping a bespoke table at a top-level `/admissions` while the design-system + `/students/*` convention already existed only as intent — now consolidated.)_

11. **Navigation is instant — never blocked on data.** A click must paint within a frame: the destination's chrome and a **skeleton of its real shape**, then the data streams in behind it. A user who clicks and sees nothing change assumes the click was lost and clicks again. Concretely, on the App Router: **every segment that `await`s needs a Suspense boundary above it.** A segment's own `loading.tsx` covers its `page.tsx` but **NOT its `layout.tsx`** — a layout wraps the loading UI, so a layout's `await` (a `requirePermission` / `getSession` call is a real `/auth/me` round trip) can only be covered by a `loading.tsx` in an **ancestor** segment. Chrome that is the same on every tab of a record belongs in that record's `layout.tsx`, so moving between tabs re-renders only the body — never re-fetch and repaint a header the user is already looking at. Skeletons come from `@workspace/ui/custom/states/page-skeletons` and must match the content's shape (use `DetailBodySkeleton` for a body nested inside a layout that already painted the header, so a second `ShellMain` is not opened inside the first). _Enforcement:_ seven gates under `scripts/audit-*.mjs` (boundaries · skeleton shapes · **skeleton fidelity** · loading states · first-load owners · route-skeleton registry · **page size**). Fidelity is the one that catches a placeholder which is the right KIND but the wrong size — four stat cards where the page shows five, five table columns where it has eight, tracked in [`action-plan/instant-navigation-audit.md`](action-plan/instant-navigation-audit.md). Re-run them after adding a route, an `async` layout, or a nav destination.
